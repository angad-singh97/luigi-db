...
static int convertCluster(std::string cluster) {
  if (cluster == LOCALHOST_CENTER)
    return LOCALHOST_CENTER_INT;
  else if (cluster == LEARNER_CENTER)
    return LEARNER_CENTER_INT;
  else if (cluster == P1_CENTER)
    return P1_CENTER_INT;
  else if (cluster == P2_CENTER)
    return P2_CENTER_INT;
  else {
    Panic("cluster name is not matched in configuration, got: %s!",
          cluster.c_str());
    return -1;
  }
}
...
static std::string convertClusterRole(int clusterRole) {
  if (clusterRole == LOCALHOST_CENTER_INT)
    return LOCALHOST_CENTER;
  else if (clusterRole == LEARNER_CENTER_INT)
    return LEARNER_CENTER;
  else if (clusterRole == P1_CENTER_INT)
    return P1_CENTER;
  else if (clusterRole == P2_CENTER_INT)
    return P2_CENTER;
  else {
    Panic("cluster role is not matched in configuration, got: %d!",
          clusterRole);
    return "";
  }
}
...
// ----------- several utils
static std::string printStringAsBit(const char *str, size_t len,
                                    std::string prefix = "") {
...
inline std::string Encode(const std::string &value) {
  ...
  *time_term = 0; // Redundant but explicit
  ...
  Node *node = reinterpret_cast<Node *>(encoded_value.data() +
                                        encoded_value.size() - BITS_OF_NODE);
  node->timestamp = 0;
  node->data_size = 0;
  node->data = nullptr;
...
struct batch_lock_request_t {
  ...
  uint32_t req_nr;
  uint16_t batch_size;
  char data[sizeof(uint16_t) * max_batch_size // the table_id sequence
            + (sizeof(uint16_t) + max_key_length) *
                  max_batch_size // the (klen, kdata) sequence
            + (sizeof(uint16_t) + max_value_length) *
                  max_batch_size // the (vlen, vdata) sequence
  ];
};
...
public:
  ...
  void set_req_nr(uint64_t req_nr) { request->req_nr = req_nr; }

  size_t get_msg_len() {
    return msg_len + offsetof(batch_lock_request_t, data);
  }

  batch_lock_request_t *get_request_ptr() { return request; }
  ...
  void read_one_request(char **key, uint16_t *klen, char **value,
                        uint16_t *vlen, uint16_t *table_id) {
    auto bytes_shift = sizeof(uint16_t);
...
using resp_continuation_t = std::function<void(char *respBuf)>;
using basic_continuation_t = std::function<void(char *respBuf)>;
