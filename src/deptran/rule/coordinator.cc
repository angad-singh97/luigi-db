  ...
  switch (phase_++ % n_phase) {
    case Phase::INIT_END:
      ...
      if (go_to_fastpath_) {
        ...
        // }
#ifdef JETPACK_FASTPATH_DEBUG
        Log_info("Before BroadcastRuleSpeculativeExecute");
#endif
        BroadcastRuleSpeculativeExecute(phase_cp);
#ifdef JETPACK_FASTPATH_DEBUG
        Log_info("After BroadcastRuleSpeculativeExecute");
#endif
      } else {
        // Do nothing
      }
...
void CoordinatorRule::BroadcastRuleSpeculativeExecute(int phase) {
  ...
  if (phase != phase_) {
#ifdef JETPACK_FASTPATH_DEBUG
    Log_info("Fast path catch up fail!");
#endif
    return;
  }
  if (fast_path_success_)
#ifdef JETPACK_FASTPATH_DEBUG
    Log_info("Fast path catch up success!");
#endif
    GotoNextPhase();
}
...
void CoordinatorRule::DispatchAsync(bool fastpath_broadcast_mode) {
  ...
  auto txn = (TxData*) cmd_;
#ifdef JETPACK_FASTPATH_DEBUG
  Log_info("DispatchAsync <%d, %d>", txn->client_id_, txn->cmd_id_in_client_);
#endif
