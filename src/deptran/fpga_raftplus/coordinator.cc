...
CoordinatorFpgaRaftPlus::CoordinatorFpgaRaftPlus(uint32_t coo_id,
                                             int32_t benchmark,
...
bool CoordinatorFpgaRaftPlus::IsLeader() {
   return this->sch_->IsLeader() ;
}

bool CoordinatorFpgaRaftPlus::IsFPGALeader() {
   return this->sch_->IsFPGALeader() ;
}

void CoordinatorFpgaRaftPlus::Forward(shared_ptr<Marshallable>& cmd,
...
void CoordinatorFpgaRaftPlus::Submit(shared_ptr<Marshallable>& cmd,
                                   const function<void()>& func,
                                   const function<void()>& exe_callback) {
  if (!IsLeader()) {
...
void CoordinatorFpgaRaftPlus::AppendEntries() {
    ...
    auto sp_quorum = commo()->BroadcastAppendEntries(par_id_,
                                                     ...
                                                     cmd_,
                                                     sch_->CurpAttemptCommitFinish(cmd_),
                                                     sch_);
...
void CoordinatorFpgaRaftPlus::LeaderLearn() {
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    commit_callback_();
...
void CoordinatorFpgaRaftPlus::GotoNextPhase() {
  int n_phase = 4;
