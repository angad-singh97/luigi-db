...
bool FpgaRaftPlusServer::looping = false;

struct hb_loop_args_type {
	FpgaRaftPlusCommo* commo;
	FpgaRaftPlusServer* sch;
};

FpgaRaftPlusServer::FpgaRaftPlusServer(Frame * frame) {
  frame_ = frame ;
...
void FpgaRaftPlusServer::Setup() {
	if (heartbeat_ && !FpgaRaftPlusServer::looping && IsLeader()) {
		Log_info("starting loop at server");
		FpgaRaftPlusServer::looping = true;
		memset(&loop_th_, 0, sizeof(loop_th_));
		hb_loop_args_type* hb_loop_args = new hb_loop_args_type();
		hb_loop_args->commo = (FpgaRaftPlusCommo*) commo();
		hb_loop_args->sch = this;
		verify(hb_loop_args->commo && hb_loop_args->sch);
		Pthread_create(&loop_th_, nullptr, FpgaRaftPlusServer::HeartbeatLoop, hb_loop_args);
	}
}

void* FpgaRaftPlusServer::HeartbeatLoop(void* args) {
	hb_loop_args_type* hb_loop_args = (hb_loop_args_type*) args;

	FpgaRaftPlusServer::looping = true;
	while(FpgaRaftPlusServer::looping) {
		usleep(100*1000);
...
FpgaRaftPlusServer::~FpgaRaftPlusServer() {
		if (heartbeat_ && FpgaRaftPlusServer::looping) {
			FpgaRaftPlusServer::looping = false;
...
void FpgaRaftPlusServer::RequestVote2FPGA() {
  ...
  auto sp_quorum = ((FpgaRaftPlusCommo *)(this->commo_))->BroadcastVote2FPGA(par_id,lst_idx,lst_term,loc_id, currentTerm );
  sp_quorum->Wait();
...
void FpgaRaftPlusServer::OnVote2FPGA(const slotid_t& lst_log_idx,
...
bool FpgaRaftPlusServer::RequestVote() {
  ...
  auto sp_quorum = ((FpgaRaftPlusCommo *)(this->commo_))->BroadcastVote(par_id,lst_idx,lst_term,loc_id, currentTerm );
  sp_quorum->Wait();
  std::lock_guard<std::recursive_mutex> lock1(mtx_);
  if (sp_quorum->Yes()) {
    // become a leader
    ...
    ((CoordinatorFpgaRaftPlus*)co)->Submit(sp_m);
...
void FpgaRaftPlusServer::OnVote(const slotid_t& lst_log_idx,
                            const ballot_t& lst_log_term,
                            const parid_t& can_id,
                            const ballot_t& can_term,
                            ballot_t *reply_term,
...
void FpgaRaftPlusServer::StartTimer()
{
    if(!init_ ){
        resetTimer() ;
        Coroutine::CreateRun([&]() {
 ...
 * should we exclude the execution of this function for leader? */
  void FpgaRaftPlusServer::OnAppendEntries(const slotid_t slot_id,
        ...
        if ((leaderCurrentTerm >= this->currentTerm) &&
            ...
            *followerLastLogIndex = this->lastLogIndex;
              ...
              auto sp_vec_piece = dynamic_pointer_cast<VecPieceData>(p_cmd->cmd_)->sp_vec_piece_data_;
							...
							vector<struct FpgaRaftPlusKeyValue> kv_vector;
							int index = 0;
							for (auto it = sp_vec_piece->begin(); it != sp_vec_piece->end(); it++){
								...
								for (auto it2 = cmd_input->begin(); it2 != cmd_input->end(); it2++) {
									struct FpgaRaftPlusKeyValue key_value = {it2->first, it2->second.get_i32()};
    ...
    void FpgaRaftPlusServer::OnForward(shared_ptr<Marshallable> &cmd, 
                                          uint64_t *cmt_idx,
                                          const function<void()> &cb) {
        this->rep_frame_ = this->frame_ ;
        auto co = ((TxLogServer *)(this))->CreateRepCoord(0);
        ((CoordinatorFpgaRaftPlus*)co)->Submit(cmd);
        
        std::lock_guard<std::recursive_mutex> lock(mtx_);
        *cmt_idx = ((CoordinatorFpgaRaftPlus*)co)->cmt_idx_ ;
  ...
  void FpgaRaftPlusServer::OnCommit(const slotid_t slot_id,
    ...
    for (slotid_t id = executeIndex + 1; id <= commitIndex; id++) {
        ...
        if (next_instance->log_) {
            ...
            CurpSkipFastpath(curp_unique_original_cmd_id_++, next_instance->log_);
  ...
  void FpgaRaftPlusServer::SpCommit(const uint64_t cmt_idx) {
  ...
  void FpgaRaftPlusServer::removeCmd(slotid_t slot) {
