...
struct FpgaRaftPlusKeyValue {
	int key;
	i32 value;
};

class FpgaRaftPlusServer : public TxLogServer {
 private:
 ...
 public:
  ...
  map<slotid_t, shared_ptr<FpgaRaftPlusData>> logs_{};
  int n_vote_ = 0;
  int n_prepare_ = 0;
  int n_accept_ = 0;
  int n_commit_ = 0;

  /* NOTE: I think I should move these to the FpgaRaftPlusData class */
  /* TODO: talk to Shuai about it */
  uint64_t lastLogIndex = 0;
  uint64_t currentTerm = 0;
  uint64_t commitIndex = 0;
  uint64_t executeIndex = 0;
  map<slotid_t, shared_ptr<FpgaRaftPlusData>> raft_logs_{};
//  vector<shared_ptr<FpgaRaftPlusData>> raft_logs_{};
  ...
  shared_ptr<FpgaRaftPlusData> GetInstance(slotid_t id) {
    verify(id >= min_active_slot_);
    auto& sp_instance = logs_[id];
    if(!sp_instance)
      sp_instance = std::make_shared<FpgaRaftPlusData>();
    return sp_instance;
  }

 /* shared_ptr<FpgaRaftPlusData> GetFpgaRaftInstance(slotid_t id) {
    if ( id <= raft_logs_.size() )
    {
        return raft_logs_[id-1] ;
    }
    auto sp_instance = std::make_shared<FpgaRaftPlusData>();
  ...
  FpgaRaftPlusServer(Frame *frame) ;
  ~FpgaRaftPlusServer() ;
