...
struct LuigiTxnRequest {
    ...
    std::function<void(int status, uint64_t commit_ts, 
                       const std::vector<std::string>& results)> callback;
...
public:
    LuigiTxnGenerator(const TxnGeneratorConfig& config)
        : config_(config) {
     ...
     */
    virtual void GetTxnReq(LuigiTxnRequest* req, uint32_t req_id, uint32_t client_id) = 0;
