 ...
 * 
 * Generates simple read/write transactions matching Mako's simpleTransaction tests.
 * Uses uniform random key distribution.
 */
...
public:
    ...
    void GetTxnReq(LuigiTxnRequest* req, uint32_t req_id, uint32_t client_id) override {
        ...
        for (uint32_t sid = 0; sid < config_.shard_num; sid++) {
            ...
            req->target_shards.insert(KeyToShard(key));
...
public:
    SingleShardMicroTxnGenerator(const TxnGeneratorConfig& config, uint32_t shard_id = 0)
