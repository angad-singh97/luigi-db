...
public:
    LuigiStateMachine(uint32_t shard_id, uint32_t replica_id,
                      uint32_t shard_num, uint32_t replica_num)
        : shard_id_(shard_id), replica_id_(replica_id),
          shard_num_(shard_num), replica_num_(replica_num) {}
     ...
     * @param ops Vector of operations to execute
     ...
     */
    virtual void MapOpsToShards(uint32_t txn_type,
                                const std::vector<LuigiOp>& ops,
...
protected:
    ...
    virtual uint32_t KeyToShard(const std::string& key) const {
        if (shard_num_ == 0) return 0;
        uint32_t hash = 0;
        for (char c : key) {
            hash = hash * 31 + static_cast<uint8_t>(c);
        }
        return hash % shard_num_;
    }
};
...
public:
    ...
    void MapOpsToShards(uint32_t txn_type,
                        const std::vector<LuigiOp>& ops,
    ...
    bool ExecuteNewOrder(const std::vector<LuigiOp>& ops,
                         std::map<std::string, std::string>* output,
                         uint64_t txn_id);
    
    bool ExecutePayment(const std::vector<LuigiOp>& ops,
                        std::map<std::string, std::string>* output,
                        uint64_t txn_id);
    
    bool ExecuteOrderStatus(const std::vector<LuigiOp>& ops,
                            std::map<std::string, std::string>* output,
                            uint64_t txn_id);
    
    bool ExecuteDelivery(const std::vector<LuigiOp>& ops,
                         std::map<std::string, std::string>* output,
                         uint64_t txn_id);
    
    bool ExecuteStockLevel(const std::vector<LuigiOp>& ops,
                           std::map<std::string, std::string>* output,
                           uint64_t txn_id);
    ...
    void SetConfig(uint32_t warehouses, uint32_t districts, 
                   uint32_t customers, uint32_t items) {
