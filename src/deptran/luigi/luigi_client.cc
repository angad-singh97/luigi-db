...
LuigiDispatchBuilder& LuigiDispatchBuilder::operator=(LuigiDispatchBuilder&& other) noexcept {
    if (this != &other) {
        delete request_;
...
LuigiDispatchBuilder& LuigiDispatchBuilder::SetExpectedTime(uint64_t expected_time) {
    request_->expected_time = expected_time;
    return *this;
}

LuigiDispatchBuilder& LuigiDispatchBuilder::SetTargetServer(uint16_t server_id) {
...
LuigiDispatchBuilder& LuigiDispatchBuilder::SetInvolvedShards(
...
LuigiClient::LuigiClient(const std::string& config_file,
                         mako::Transport* transport,
                         uint64_t client_id)
    : config_(config_file),
...
void LuigiClient::InvokeDispatch(
    ...
    if (!requests_per_shard.empty()) {
        server_id = requests_per_shard.begin()->second->GetRequest()->target_server_id;
    }
    ...
    for (auto& kv : requests_per_shard) {
        ...
        data_to_send[shard_idx] = {
            reinterpret_cast<char*>(builder->GetRequest()),
    ...
    transport_->SendBatchRequestToAll(
...
void LuigiClient::HandleDispatchReply(char* respBuf) {
    ...
    Log_debug("Luigi dispatch reply: req_nr=%d, txn_id=%lu, status=%d, commit_ts=%lu",
...
void LuigiClient::InvokeStatusCheck(
    uint64_t txn_nr,
    uint64_t txn_id,
    ...
    Log_debug("InvokeStatusCheck: txn_id=%lu, num_shards=%zu", txn_id, shard_indices.size());
    ...
    transport_->SendBatchRequestToAll(
        nullptr,
        luigi::kLuigiStatusReqType,
        config_.warehouses + 5,
        sizeof(luigi::StatusResponse),
        data_to_send
    );
...
void LuigiClient::InvokeOwdPing(
    uint64_t txn_nr,
    int shard_idx,
    ResponseCallback continuation,
    ErrorCallback error_continuation,
    ...
    transport_->SendRequestToShard(
        nullptr,
        luigi::kOwdPingReqType,
        shard_idx,
        config_.warehouses + 5,
        sizeof(luigi::OwdPingRequest));
}
