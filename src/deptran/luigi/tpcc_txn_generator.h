...
public:
    ...
    std::string RTTI() override {
        return "TPCCTxnGenerator";
    }
    
    void GetTxnReq(LuigiTxnRequest* req, uint32_t req_id, uint32_t client_id) override {
...
private:
    ...
    void GetNewOrderTxn(LuigiTxnRequest* req) {
        ...
        int32_t d_id = (req->client_id / config_.num_warehouses) % config_.num_districts_per_wh;
        ...
        // Build ops for warehouse and district reads
        req->ops.push_back(MakeOp(TPCC_TABLE_WAREHOUSE, 0, 
                                   std::to_string(home_w_id), ""));
        req->ops.push_back(MakeOp(TPCC_TABLE_DISTRICT, 1,  // write for D_NEXT_O_ID
                                   DistrictKey(home_w_id, d_id), ""));
        
        // Customer read
        req->ops.push_back(MakeOp(TPCC_TABLE_CUSTOMER, 0,
                                   CustomerKey(home_w_id, d_id, c_id), ""));
        ...
        for (int32_t i = 0; i < ol_cnt; i++) {
            ...
            if (config_.num_warehouses > 1 && RandomInt(0, 100) == 0) {
                ...
                if (supply_w_id >= home_w_id) supply_w_id++;
                all_local = false;
            } else {
            ...
            // Item read (items are replicated, read from any shard)
            req->ops.push_back(MakeOp(TPCC_TABLE_ITEM, 0,
                                       std::to_string(i_id), ""));
            
            // Stock update
            req->ops.push_back(MakeOp(TPCC_TABLE_STOCK, 1,
                                       StockKey(supply_w_id, i_id), 
                                       std::to_string(quantity)));
        }
        
        req->working_set[TPCC_VAR_O_ALL_LOCAL] = all_local ? "1" : "0";
        
        // New Order, Order, Order Line inserts
        req->ops.push_back(MakeOp(TPCC_TABLE_NEW_ORDER, 1, 
                                   NewOrderKey(home_w_id, d_id), ""));
        req->ops.push_back(MakeOp(TPCC_TABLE_ORDER, 1,
                                   OrderKey(home_w_id, d_id), ""));
    }
    ...
    void GetPaymentTxn(LuigiTxnRequest* req) {
        ...
        // Build ops
        req->ops.push_back(MakeOp(TPCC_TABLE_WAREHOUSE, 1,
                                   std::to_string(home_w_id), std::to_string(h_amount)));
        req->ops.push_back(MakeOp(TPCC_TABLE_DISTRICT, 1,
                                   DistrictKey(home_w_id, d_id), std::to_string(h_amount)));
        req->ops.push_back(MakeOp(TPCC_TABLE_CUSTOMER, 1,
                                   CustomerKey(c_w_id, c_d_id, 0), std::to_string(h_amount)));
        req->ops.push_back(MakeOp(TPCC_TABLE_HISTORY, 1,
    ...
    void GetOrderStatusTxn(LuigiTxnRequest* req) {
        ...
        // Read customer, order, order_line
        req->ops.push_back(MakeOp(TPCC_TABLE_CUSTOMER, 0,
                                   CustomerKey(w_id, d_id, 0), ""));
        req->ops.push_back(MakeOp(TPCC_TABLE_ORDER, 0,
                                   OrderKey(w_id, d_id), ""));
        req->ops.push_back(MakeOp(TPCC_TABLE_ORDER_LINE, 0,
                                   OrderLineKey(w_id, d_id), ""));
    }
    ...
    void GetDeliveryTxn(LuigiTxnRequest* req) {
        ...
        for (int32_t d_id = 0; d_id < static_cast<int32_t>(config_.num_districts_per_wh); d_id++) {
            req->working_set[TPCC_VAR_D_ID] = std::to_string(d_id);
            
            req->ops.push_back(MakeOp(TPCC_TABLE_NEW_ORDER, 1,
                                       NewOrderKey(w_id, d_id), ""));
            req->ops.push_back(MakeOp(TPCC_TABLE_ORDER, 1,
                                       OrderKey(w_id, d_id), std::to_string(carrier_id)));
            req->ops.push_back(MakeOp(TPCC_TABLE_CUSTOMER, 1,
                                       CustomerKey(w_id, d_id, 0), ""));
        }
    }
    ...
    void GetStockLevelTxn(LuigiTxnRequest* req) {
        ...
        // Read district to get next O_ID
        req->ops.push_back(MakeOp(TPCC_TABLE_DISTRICT, 0,
                                   DistrictKey(w_id, d_id), ""));
        // Read recent order lines
        req->ops.push_back(MakeOp(TPCC_TABLE_ORDER_LINE, 0,
                                   OrderLineKey(w_id, d_id), ""));
        // Read stock
        req->ops.push_back(MakeOp(TPCC_TABLE_STOCK, 0,
                                   std::to_string(w_id), std::to_string(threshold)));
    }
    
private:
    // NURand function per TPC-C spec
    int32_t NURand(int32_t A, int32_t x, int32_t y) {
        return (((RandomInt(0, A) | RandomInt(x, y + 1)) + 
                 RandomInt(0, A)) % (y - x + 1)) + x;
    }
    ...
    std::string CustomerKey(int32_t w_id, int32_t d_id, int32_t c_id) {
        return std::to_string(w_id) + ":" + std::to_string(d_id) + ":" + std::to_string(c_id);
    }
