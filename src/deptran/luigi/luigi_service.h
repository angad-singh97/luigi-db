 ...
 *
 * This service implements the LuigiLeaderService interface defined in
 * rcc_rpc.rpc. Each leader runs this service to receive deadline proposals from
 * other leaders.
 *
 * Protocol:
...
public:
   ...
   */
  void DeadlineConfirm(const LuigiDeadlineRequest &req, rrr::i32 *status,
                       rrr::DeferredReply *defer) override;

  /**
   * Handle incoming watermark update from another leader.
   *
   * Updates the global watermark view for the source shard.
   */
  void WatermarkExchange(const LuigiWatermarkRequest &req,
