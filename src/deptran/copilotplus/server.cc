...
CopilotPlusServer::CopilotPlusServer(Frame* frame) : log_infos_(2) {
  frame_ = frame;
...
shared_ptr<CopilotPlusData> CopilotPlusServer::GetInstance(slotid_t slot, uint8_t is_pilot) {
...
std::pair<slotid_t, uint64_t> CopilotPlusServer::PickInitSlotAndDep() {
...
slotid_t CopilotPlusServer::GetMaxCommittedSlot(uint8_t is_copilot) {
  return log_infos_[is_copilot].max_committed_slot;
}

bool CopilotPlusServer::WaitMaxCommittedGT(uint8_t is_pilot, slotid_t slot, int timeout) {
  auto &event = log_infos_[is_pilot].max_cmit_evt;
  event.WaitUntilGreaterOrEqualThan(slot, timeout);
  return (event.value_ >= slot);
}

bool CopilotPlusServer::allCmdComitted(shared_ptr<Marshallable> batch_cmd) {
...
bool CopilotPlusServer::EliminateNullDep(shared_ptr<CopilotPlusData> &ins) {
  verify(ins);
  auto& cmd = ins->cmd;
...
bool CopilotPlusServer::AllDepsEliminated(uint8_t is_pilot, slotid_t dep_id) {
...
void CopilotPlusServer::WaitForPingPong() {
  /**
...
bool CopilotPlusServer::WillWait(int &time_to_wait) const {
  /**
...
void CopilotPlusServer::OnForward(shared_ptr<Marshallable>& cmd,
  ...
  auto coord = (CopilotPlusCoordinator *)CreateRepCoord();
  coord->Submit(cmd);

  cb();
}

void CopilotPlusServer::OnPrepare(const uint8_t& is_pilot,
                              const uint64_t& slot,
...
void CopilotPlusServer::OnFastAccept(const uint8_t& is_pilot,
                                 const uint64_t& slot,
                                 const ballot_t& ballot,
                                 const uint64_t& dep,
...
void CopilotPlusServer::OnAccept(const uint8_t& is_pilot,
                             const uint64_t& slot,
                             const ballot_t& ballot,
                             const uint64_t& dep,
                             shared_ptr<Marshallable>& cmd,
...
void CopilotPlusServer::setIsPilot(bool isPilot) {
...
void CopilotPlusServer::setIsCopilot(bool isCopilot) {
...
inline void CopilotPlusServer::updateMaxExecSlot(shared_ptr<CopilotPlusData>& ins) {
...
void CopilotPlusServer::updateMaxAcptSlot(CopilotPlusLogInfo& log_info, slotid_t slot) {
  slotid_t i;
...
void CopilotPlusServer::updateMaxCmtdSlot(CopilotPlusLogInfo& log_info, slotid_t slot) {
  slotid_t i;
...
void CopilotPlusServer::removeCmd(CopilotPlusLogInfo& log_info, slotid_t slot) {
...
#ifdef USE_TARJAN
bool CopilotPlusServer::executeCmds(shared_ptr<CopilotPlusData>& ins) {
...
bool CopilotPlusServer::checkAllDepExecuted(uint8_t is_pilot, slotid_t start, slotid_t end) {
...
bool CopilotPlusServer::isExecuted(shared_ptr<CopilotPlusData>& ins) {
...
bool CopilotPlusServer::findSCC(shared_ptr<CopilotPlusData>& root) {
  int index = 1;
  while (!stack_.empty()) {
    // auto v = stack_.top();
    // v->dfn = 0;
...
bool CopilotPlusServer::strongConnect(shared_ptr<CopilotPlusData>& ins, int* index) {
  ins->dfn = *index;
  ins->low = *index;
  *index = *index + 1;
  stack_.push(ins);
  ...
  if (ins->low == ins->dfn) {
    std::vector<shared_ptr<CopilotPlusData> > list;
    shared_ptr<CopilotPlusData> w;
    ...
    std::sort(list.begin(), list.end(),
              [](const shared_ptr<CopilotPlusData>& i1,
                 const shared_ptr<CopilotPlusData>& i2) -> bool {
 ...
 */
void CopilotPlusServer::waitAllPredCommit(shared_ptr<CopilotPlusData>& ins) {
 ...
 * DFS has an independent visited map.
 */
void CopilotPlusServer::waitPredCmds(shared_ptr<CopilotPlusData>& w, shared_ptr<visited_map_t> m) {
  (*m)[w] = true;
  auto pre_ins = GetInstance(w->slot_id - 1, w->is_pilot);
  auto dep_ins = GetInstance(w->dep_id, REVERSE(w->is_pilot));
