...
const char* indicator_plus[] = {"COPILOT", "PILOT"};

bool FreeDanglingPlus(Communicator* comm, vector<std::pair<uint16_t, rrr::i64> > &dangling) {
...
inline ballot_t CopilotPlusCoordinator::makeUniqueBallot(ballot_t ballot) {
  /**
   * ballot format:
   * 63           8 7        0
...
inline ballot_t CopilotPlusCoordinator::pickGreaterBallot(ballot_t ballot) {
...
void CopilotPlusCoordinator::Prepare() {
  std::lock_guard<std::recursive_mutex> lock(mtx_);
start_prepare:
  static_cast<CopilotPlusFrame*>(frame_)->n_prepare_++;
  ...
  Log_debug(
      ...
      par_id_, indicator_plus[is_pilot_], slot_id_, new_ballot);
  // sq_quorum->id_ = dep_id_;

  sq_quorum->Wait();
#ifdef DO_FINALIZE
  sq_quorum->Finalize(finalize_timeout_us,
                      std::bind(FreeDanglingPlus, commo(), std::placeholders::_1));
#endif
  // sq_quorum->log();
  ...
  if (sq_quorum->committed_seen_) {
    ...
    Log_warn("%s : %lu Prepare failed, retry",
              indicator_plus[is_pilot_], slot_id_);
    goto start_prepare;
  }
...
void CopilotPlusCoordinator::FastAccept() {
  ...
  if ((static_cast<CopilotPlusFrame*>(frame_)->n_fast_accept_ & 0xfff) == 0)
      Log_info("fast/reg/total %u/%u/%u", static_cast<CopilotPlusFrame*>(frame_)->n_fast_path_,
        static_cast<CopilotPlusFrame*>(frame_)->n_regular_path_,
        static_cast<CopilotPlusFrame*>(frame_)->n_fast_accept_);
  Log_debug(
      "Copilot coordinator %u broadcast FAST_ACCEPT, "
      "partition: %u, %s : %lu -> %lu",
      coo_id_, par_id_, indicator_plus[is_pilot_], slot_id_, dep_);
  ...
  if (sq_quorum->FastYes()) {
    ...
    static_cast<CopilotPlusFrame*>(frame_)->n_fast_path_++;
...
void CopilotPlusCoordinator::Accept() {
  ...
  // cout << "ac";
  // sp_quorum->Log();
  // if ((static_cast<CopilotPlusFrame*>(frame_)->n_accept_ & 0x3ff) == 0)
...
void CopilotPlusCoordinator::Commit() {
  ...
  Log_debug("Copilot coordinator %u broadcast COMMIT for partition: %d, %s : %lu -> %lu",
            coo_id_, (int)par_id_, indicator_plus[is_pilot_], slot_id_, dep_);
  ...
  // if (false) {
    ...
    if (!sch_->AllDepsEliminated(REVERSE(is_pilot_), dep_) &&
      ...
      */
      ...
      Log_info("TAKEOVER on %s for %lu from %lu to %lu", indicator_plus[cur_pilot], cur_slot, start, end);
      for (auto i = start; i <= end; i++) {
        ...
        if (ucmit_ins
          ...
          Log_info(
              ...
              " %s, %lu, status: %x",
              indicator_plus[cur_pilot], cur_slot, indicator_plus[ucmit_ins->is_pilot],
...
void CopilotPlusCoordinator::initFastTakeover(shared_ptr<CopilotPlusData>& ins) {
...
inline void CopilotPlusCoordinator::clearStatus() {
  if (done_)
    return;
  done_ = true;
  curr_ballot_ = 0;
