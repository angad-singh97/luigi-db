...
Coordinator *CopilotPlusFrame::CreateCoordinator(cooid_t coo_id,
  ...
  auto coord = new CopilotPlusCoordinator(coo_id, benchmark, ccsi, id);
...
TxLogServer *CopilotPlusFrame::CreateScheduler() {
  if (sch_ == nullptr) {
    sch_ = new CopilotPlusServer(this);
  } else {
    verify(0);
  }
...
vector<rrr::Service *>
CopilotPlusFrame::CreateRpcServices(uint32_t site_id,
...
void CopilotPlusFrame::setupCoordinator(CopilotPlusCoordinator *coord, Config *config) {
  coord->frame_ = this;
