...
void MenciusPlusServiceImpl::Suggest(const uint64_t& slot,
  ...
  int n = Config::GetConfig()->GetPartitionSize(sched()->partition_id_);
  sched()->skip_potentials_recd[(slot-1)%n].clear();
  for (auto x: skip_potentials){
    sched()->skip_potentials_recd[(slot-1)%n].insert(x);
  }
  ...
  for (auto x: skip_commits){
    ...
    sched()->OnCommit(x, 100, md.sp_data_, true);
  }
  sched()->g_mutex.unlock();
