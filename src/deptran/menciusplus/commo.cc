...
MenciusPlusCommo::MenciusPlusCommo(PollMgr* poll) : Communicator(poll) {
//  verify(poll != nullptr);
}

void MenciusPlusCommo::BroadcastPrepare(parid_t par_id,
                                       slotid_t slot_id,
                                       ballot_t ballot,
...
MenciusPlusCommo::BroadcastPrepare(parid_t par_id,
                                  slotid_t slot_id,
                                  ballot_t ballot) {
  verify(0);
  int n = Config::GetConfig()->GetPartitionSize(par_id);
  auto e = Reactor::CreateSpEvent<MenciusPlusPrepareQuorumEvent>(n, n/2+1);
...
//  auto e = Reactor::CreateSpEvent<MenciusPlusSuggestQuorumEvent>(n, n);
...
void MenciusPlusCommo::BroadcastSuggest(parid_t par_id,
                                      slotid_t slot_id,
                                      ballot_t ballot,
                                      shared_ptr<Marshallable> cmd,
                                      const function<void(Future*)>& cb) {
...
void MenciusPlusCommo::BroadcastDecide(const parid_t par_id,
