...
CoordinatorMultiPaxosPlus::CoordinatorMultiPaxosPlus(
    uint32_t coo_id,
    int32_t benchmark,
...
void CoordinatorMultiPaxosPlus::Submit(shared_ptr<Marshallable>& cmd,
    ...
    Log_info("[CURP] Paxos Submit cmd<%d, %d>",
             SimpleRWCommand::GetCmdID(cmd).first,
             SimpleRWCommand::GetCmdID(cmd).second);
#endif
#ifdef LATENCY_DEBUG
    sch_->cli2leader_recv_.append(SimpleRWCommand::GetCommandMsTimeElaps(cmd));
#endif
...
void CoordinatorMultiPaxosPlus::Prepare() {
    ...
    // rpc_event->log();
    Log_debug(
        "multi-paxos coordinator broadcasts prepare, "
    ...
    Log_info(
        "Time before Wait() is: %d",
        chrono::duration_cast<chrono::milliseconds>(start.time_since_epoch())
            .count());
    sp_quorum->Wait();
...
void CoordinatorMultiPaxosPlus::Accept() {
    ...
    Log_info("[CURP] Paxos Accept cmd<%d, %d>",
             SimpleRWCommand::GetCmdID(cmd_).first,
             SimpleRWCommand::GetCmdID(cmd_).second);
#endif
    verify(!in_accept);
    in_accept = true;
    Log_debug(
        "multi-paxos coordinator broadcasts accept, "
        "par_id_: %lx, slot_id: %llx",
        par_id_, slot_id_);
    ...
    // auto duration_ready =
    // chrono::duration_cast<chrono::microseconds>(end-sp_quorum->ready_time);
    // Log_info("Duration of Wait() in Accept() is: %d", duration.count());
    // Log_info("Duration after Ready to end of Wait() is: %d",
    // duration_ready.count());
    sp_quorum->log();
    if (sp_quorum->Yes()) {
        committed_ = true;
    } else if (sp_quorum->No()) {
...
#ifdef LATENCY_DEBUG
    sch_->cli2commit_send_.append(SimpleRWCommand::GetCommandMsTimeElaps(cmd_));
#endif
